...
def task_extract_key_points(blog_post):
    """Task function to extract key points from a blog post."""
    messages = [
...
def task_generate_summary(key_points, max_length=150):
    """Task function to generate a concise summary from key points."""
    messages = [
...
def task_create_social_media_posts(key_points, blog_title):
    """Task function to create social media posts for different platforms."""
    messages = [
...
def task_create_email_newsletter(blog_post, summary, key_points):
    """Task function to create an email newsletter."""
    messages = [
...
def improve_content(content, feedback, content_type):
    ...
    messages = [
        {"role": "system", "content": f"You are an expert at refining {content_type}s. Use the feedback to improve the content. Return the improved content in the same format as the original (e.g., Subject: [text]\nBody: [text] for email_newsletter)."},
        {"role": "user", "content": f"Original {content_type}:\n{content if isinstance(content, str) else json.dumps(content)}\n\nFeedback:\n{feedback}\n\nProvide an improved version of the {content_type}."}
    ]
    response = call_llm(messages)
    if response and response.choices[0].message.content:
...
if __name__ == "__main__":
    ...
    if blog_post:
        ...
        print("\nRunning Comparative Evaluation System (Bonus Challenge):")
